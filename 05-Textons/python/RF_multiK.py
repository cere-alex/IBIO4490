...
for fac in range(1,20):
    ...
    for idx in range(imageAmount):
        ...
        pred_tr.append(clf.predict(auxiliarArray))
    ...
    for idx in range(testImages):
        ...
        pred_tes.append(clf.predict(auxiliarArray))
...
with open('ACAs_train_RF.pickle','wb') as handle:
    pickle.dump(aux, handle, protocol=pickle.HIGHEST_PROTOCOL)
aux={}
aux['ACAs_test_RF']=ACAs_test
with open('ACAs_train_RF.pickle','wb') as handle:
    pickle.dump(aux, handle, protocol=pickle.HIGHEST_PROTOCOL)    
